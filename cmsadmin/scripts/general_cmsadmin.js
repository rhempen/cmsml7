...
function setLanguForTranslation(iLangu, iEditId, iTemplate)
{
    ...
    if (!$(lFormId)) {
        alert('ID ' + lFormId + ' existiert nicht!');
        return false;
    }
    ...
    $(lFormId).request({
        ...
        onComplete: function () {
            lUrl = lUrl + "&msg_save=" + $("msgFromSave").value + "&led_save=" + $("ledFromSave").value;
...
var zeige_resultat = function (r)
{
    ...
    $("msgFromSave").value = r.responseText;
