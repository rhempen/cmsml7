...
class naviPresent {
    ...
    public function navigation_anzeigen($navigation, $message) {
        ...
        while ($row = $navigation->fetchRow(MDB2_FETCHMODE_ASSOC)) {
            ...
            if ($anz_ukap == 0) {
                $tpl->setVariable('sortableDivEnd', '</div>');
            }
            ...
            foreach ($sprachen as $key => $sprache) {
                $bezeich = $row['bezeich_' . $sprache];
                $bezeich = $bezeich !== null ? $bezeich : 'value';
                $tpl->setVariable('bezeich_' . $sprache, $general->reslash($bezeich));
            ...
            // Unternavigationspunkt
                ...
                if ($plus_minus == 'plus') {
                    $tpl->setVariable('ukap_hide', $disp_none);
                }
            }
