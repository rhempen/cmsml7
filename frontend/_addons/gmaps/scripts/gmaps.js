...
function initialize() {
    ...
    downloadUrl(gPhpfile, function (data) {
        ...
        gModus = anzahl.length !== 0 ? anzahl[0].getAttribute("modus") : '';
...
function doAfterRequest(iText, iStatus) {
    ...
    if (gModus == 'front')
        lErgebnis.className = 'hide';
    if (lListButt && gAnzahl != 0)
        lListButt.className = 'switch'; // Liste-Button anzeigen
    return true;
}
