...
function dbAbfrageMitUmkreis($lat, $lng) {
    global $dista, $lookForActiv, $connection;
...
function fillMarkerXml($row) {
    ...
    $html .= 'mid="' . $row['mid'] . '" ';
    $html .= 'typ="' . $row['typ'] . '" ';
    $html .= 'nam="' . parseToXML($row['nam']) . '" ';
    $html .= 'adr="' . parseToXML($row['adr']) . '" ';
    $html .= 'ort="' . parseToXML($row['ort']) . '" ';
    $html .= 'lat="' . $row['lat'] . '" ';
    $html .= 'lng="' . $row['lng'] . '" ';
    $html .= 'act="' . $row['act'] . '" ';
    $html .= '/>';
        ...
        if ($row['lat'] == '0' || $row['lng'] == '0') {
            ...
            if ($lat != '0' && $lng != '0') {
                $query = sprintf("UPDATE cms_markers2 " .
                        " SET lat = '%s', lng = '%s' " .
                        " WHERE mid = '%s' LIMIT 1;", mysqli_real_escape_string($connection, $lat), mysqli_real_escape_string($connection, $lng), mysqli_real_escape_string($connection, $mid));
...
if ($flag_not_found) {
    fillMarkerXmlNotFound($coordinates[1], $coordinates[0]);
}
// End XML file
echo '</markers>';
